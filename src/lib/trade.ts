...
export function buildTradeExport(member: string, memberName: string, give: TradeGive[], want: string[]): TradeExport {
  return {
    app: 'NogiPhotoManager46',
    kind: 'trade',
    member,
...
export function parseTradeExport(text: string): ParsedTrade {
  const d = JSON.parse(text) as Partial<TradeExport>
  ...
  return { memberName: typeof d.memberName === 'string' && d.memberName ? d.memberName : '相手', give, want }
}
