...
{"photos":[{"box_2d":[ymin,xmin,ymax,xmax],"caption_edge":string|null,"caption":string|null,"caption_confidence":number,"pose":string,"pose_confidence":number,"rarity":string}]}
...
- rarity: この生写真が通常版かレアかを、下余白の文字と表面の質感で判定する。
    ・"normal"=下余白のタイトル・メンバー名が活字（きれいな印刷フォント）で整然と印刷され、表面に星や粒のキラキラ（ホログラム）がない。
    ・"R"=下余白の文字が手書き風（マーカーで書いたような一筆書き）で、表面に星・粒のキラキラ（ホログラム）があり、下の余白やフチの地色は白い。
    ・"SR"=手書き風の文字＋キラキラで、さらに下余白やフチが虹色（レインボーのホログラム）に色づいている。
    ・見分けの要点: 活字＝normal / 手書き風＋白い下余白＝R / 手書き風＋虹色＝SR。キラキラがはっきりしなければ "normal"。
...
export default async function handler(req: any, res: any) {
  ...
  const photos = rawPhotos.slice(0, 12).map((p: any) => ({
    ...
    rarity: RARITIES.includes(p?.rarity) ? p.rarity : 'normal',
  }))
